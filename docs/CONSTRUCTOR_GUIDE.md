...
- загрузка `content/filters.json` через `ContentService.loadFilters()`;
- локальное редактирование линий, типов кейсов и категорий;
- управление связями «тип кейса ↔ линия» через поле `caseType.lines`;
- сохранение изменений в GitHub через n8n webhook `/webhook/sm/constructor/save-filters`.

## 4. Работа с калькуляторами

Реестр калькуляторов хранится в `content/calculators.json`.

Для каждого калькулятора задаются:

- `id`, `slug`, `title`;
- `n8nCalcId` — идентификатор workflow в n8n.

Страница одного калькулятора: маршрут `#calc/<slug>`, модуль `js/modules/calc/main.js`.  
Доступ к калькулятору защищён паролем и проверяется через webhook `/webhook-test/sm/calc-auth`.
